...
float3 diskToPoincareTangent(float3 x, float3 v) {
    float w = sqrt(max(0.0f, 1.0f - dot(x, x)));
    float denom = 1.0f + w;
    return v / denom + x * (dot(x, v) / max(w * denom * denom, EPS));
}
...
float4 liftTangentPoincare(float3 p, float3 v) {
    ...
    return float4((2.0f / D) * (v + (2.0f * pv / D) * p), 4.0f * pv / (D * D));
}
...
SurfaceCoeffs transformedSurfaceH3(SceneObject obj, float4x4 transform) {
    QuadraticSurface q = diskSurfaceToPoincare(float3(obj.a.x, obj.a.y, obj.a.z), obj.b, obj.c);

    AmbientSurface s = quadraticToAmbient(q, -1.0f);

    s.n = transform * s.n;

    QuadraticSurface q2 = ambientToQuadratic(s, -1.0f);
...
float4 liftTangent(float3 x, float3 v,float kappa) {
    float w = sqrt(max(0.0f, 1.0f - dot(x, x)));
...
SurfaceCoeffs transformedSurface(SceneObject obj, float4x4 transform) {
    float4 A = float4(obj.a.x, obj.a.y, obj.a.z, obj.b);

    // The shader operates in the disk model, where both H³ and S³ use S³
    // ambient isometries. A plane A·X = c transforms as A' = M A, c' = c.
    float4 Ap = transform * A;
...
Roots solveQuadratic(float a, float b, float c) {
    ...
    discriminant = max(discriminant, 0.0f);
...
float forwardOrder(float t, float radiusSin) {
    if (t <= SELF_HIT_EPS)
        return INF;
...
float3 computeSpecular(float3 position,
                       float3 normal,
                       float kappa,
                       float4x4 transform,
                       device const PointLight* lights,
    ...
    for (int i = 0; i < lightCount; ++i) {
        ...
        result += lights[i].color * lights[i].intensity * specular;
    }
...
Hit findNearestHit(float3 rayDirection,
    ...
    for (int i = 0; i < objectCount; ++i) {
        ...
        for (int rootIndex = 0; rootIndex < roots.count; ++rootIndex) {
            ...
            float order = forwardOrder(t, radiusSin);
            ...
            float lhs = a.x * t * rayDirection.x
            ...
            float3 gradient = a - (b / max(w, EPS)) * position;
            ...
            nearest.normal = gradient * rsqrt(gradient2);
        }
    }
...
float3x3 quaternionRotation(float4 q) {
    ...
    float3 c0 = float3(1.0f - 2.0f * (yy + zz), 2.0f * (xy + zw), 2.0f * (xz - yw));
    float3 c1 = float3(2.0f * (xy - zw), 1.0f - 2.0f * (xx + zz), 2.0f * (yz + xw));
    float3 c2 = float3(2.0f * (xz + yw), 2.0f * (yz - xw), 1.0f - 2.0f * (xx + yy));
...
float3x3 rotationFromTo(float3 from, float3 to) {
    ...
    float c = clamp(dot(a, b), -1.0f, 1.0f);
    ...
    if (c < -1.0f + 1e-6f) {
        float3 basis = (fabs(a.x) < 0.9f)
                        ? float3(1, 0, 0)
                        : float3(0, 1, 0);

        float3 axis = normalize(cross(a, basis));

        // 180-degree rotation:
        //
        // q = (axis, 0)
        return quaternionRotation(float4(axis, 0.0f));
    }
    ...
    float4 q = float4(cross(a, b), 1.0f + c);
    q = normalize(q);
...
float4x4 embedSpatialRotation(float3x3 R) {
    ...
    return float4x4(float4(c0.x, c0.y, c0.z, 0.0f),
...
float4x4 movePointToOrigin(float4 P, float kappa) {
    ...
    return float4x4(float4(c0.x, c0.y, c0.z, kappa * u.x),
                    float4(c1.x, c1.y, c1.z, kappa * u.y),
                    float4(c2.x, c2.y, c2.z, kappa * u.z),
                    float4(-u.x, -u.y, -u.z, w));
}
...
float4x4 makeReflectionChartChange(float3 hitPosition, float3 reflectedDirection, float3 targetRayDirection, float kappa) {
    float4 P = liftPoint(hitPosition, 1.0f);
    float4 V = liftTangent(hitPosition, normalize(reflectedDirection), 1.0f);

    // First move the hit point to the origin.
    float4x4 B = movePointToOrigin(P, 1.0f);

    // Tangent after this coordinate change.
    float4 movedV = B * V;
    float3 movedDirection = normalize(movedV.xyz);

    // Then rotate around the origin so the reflected tangent
    // becomes the original fixed ray direction.
    float3x3 R3 = rotationFromTo(movedDirection, targetRayDirection);
    float4x4 R = embedSpatialRotation(R3);
...
float4x4 makeReflectionChartChangeH3(float3 hitPosition, float3 reflectedDirection, float3 targetRayDirection) {
    float3 hitPoincare = diskToPoincare(hitPosition);
    float3 reflectedPoincare = diskToPoincareTangent(hitPosition, normalize(reflectedDirection));
    float4 P = liftPointPoincare(hitPoincare);
    float4 V = liftTangentPoincare(hitPoincare, normalize(reflectedPoincare));
    float4x4 B = movePointToOrigin(P, -1.0f);
    float4 movedV = B * V;
    float3 movedDirection = normalize(movedV.xyz);
    float3x3 R3 = rotationFromTo(movedDirection, targetRayDirection);
...
kernel void raytrace(device const uchar* packet [[buffer(0)]],
    ...
    device const PacketHeader* header = reinterpret_cast<device const PacketHeader*>(packet);
    ...
    if (header->controls.modelKind != MODEL_H3 &&
    ...
    device const SceneObject* objects = reinterpret_cast<device const SceneObject*>(packet + PACKET_HEADER_SIZE);

    uint materialOffset = PACKET_HEADER_SIZE + uint(objectCount) * OBJECT_SIZE;

    device const Material* materials = reinterpret_cast<device const Material*>(packet + materialOffset);

    uint lightOffset = materialOffset + uint(materialCount) * 32;

    device const PointLight* lights = reinterpret_cast<device const PointLight*>(packet + lightOffset);
    ...
    float3 rayDirection = normalize(header->camera.fwd.xyz
                                    + screen.x * header->camera.aspect * header->camera.fovTan * header->camera.right.xyz
    ...
    for (int bounce = 0; bounce <= maxBounces; ++bounce) {
        ...
        if (obj.kind == OBJECT_OPAQUE_SPHERE) {
            ...
            //
            //      chart distance = |t|.
            float chartDistance = fabs(hit.t);

            float falloff = 1.0f / (1.0f + header->controls.falloffK * chartDistance);

            float3 diffuseLighting = computeLighting(hit.position,
                                                     hit.normal,
                                                     kappa,
                                                     transform,
                                                     lights,
            ...
            float3 specularLighting = computeSpecular(hit.position,
                                                      hit.normal,
                                                      kappa,
                                                      transform,
                                                      lights,
            ...
            radiance += throughput * (material.color.rgb * (header->controls.ambient + falloff * diffuseLighting)
                        + falloff * material.specular.rgb * material.specular.a * specularLighting);
        ...
        if (obj.kind == OBJECT_MIRROR_SPHERE) {
            ...
            throughput *= material.specular.rgb * material.specular.a * header->controls.bounceAttenuation;

            // Because the chart metric is conformal,
            // Euclidean reflection gives the correct angle.
            float3 reflected = normalize(reflect(rayDirection,hit.normal));

            // Coordinate change:
            //
            // current chart -> next chart.
            float4x4 step = (modelKind == MODEL_H3)
                    ? makeReflectionChartChangeH3(hit.position, reflected, rayDirection)
                    : makeReflectionChartChange(hit.position, reflected, rayDirection, kappa);
    ...
    output.write(float4(clamp(radiance, float3(0.0f), float3(1.0f)), 1.0f), pixel);
}
